...
__device__ __forceinline__ void lyra2_mix(const uint8_t* header, uint32_t len, uint64_t nonce, uint32_t out[8]) {
    seed_digest(header, len, nonce, 0x5593u, out);
    #pragma unroll
    for (int r = 0; r < 6; ++r) {
        mix_digest_round(out, 0xd9177950u + r * 0x9e3779b9u);
