...
__global__ void cuckoo_cortex_generate_edges_kernel(uint32_t* workspace, size_t words, uint32_t seed) {
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    for (uint64_t i = tid; i < words; i += uint64_t(blockDim.x) * gridDim.x) {
        uint32_t x = avalanche32(uint32_t(i) ^ seed ^ 0x88200a87u);
        workspace[i] = rotl32(x + uint32_t(i * 2654435761ull), int(i & 31));
    }
}

__global__ void cuckoo_cortex_trim_edges_kernel(uint32_t* workspace, size_t words, uint32_t seed) {
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    for (uint64_t i = tid; i < words; i += uint64_t(blockDim.x) * gridDim.x) {
        uint32_t x = avalanche32(uint32_t(i) ^ seed ^ 0x346d006du);
        workspace[i] = rotl32(x + uint32_t(i * 2654435761ull), int(i & 31));
    }
}

__global__ void cuckoo_cortex_search_kernel(const MiningJob* job, MiningResult* results, unsigned int* result_count, uint32_t* checksum, const uint32_t* dataset, size_t dataset_words) {
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    uint64_t nonce = job->start_nonce + tid * uint64_t(job->nonce_count);
    uint32_t digest[8];
    cuckoo_common_mix(job->header, job->header_len, nonce, digest);
...
__global__ void cuckoo_cortex_cycle_check_kernel(uint32_t* checksum, const uint32_t* workspace, size_t words) {
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
int main(int argc, char** argv) {
    ...
    cuckoo_cortex_generate_edges_kernel<<<args.blocks, args.threads>>>(d_workspace, workspace_words, args.seed + 0x7a2u);
    CUDA_CHECK(cudaGetLastError());
    cuckoo_cortex_trim_edges_kernel<<<args.blocks, args.threads>>>(d_workspace, workspace_words, args.seed + 0x184eu);
    CUDA_CHECK(cudaGetLastError());
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        ...
        CUDA_CHECK(cudaMemcpy(d_job, &h_job, sizeof(MiningJob), cudaMemcpyHostToDevice));
        cuckoo_cortex_search_kernel<<<args.blocks, args.threads>>>(d_job, d_results, d_result_count, d_checksum, d_workspace, workspace_words);
        CUDA_CHECK(cudaGetLastError());
        cuckoo_cortex_cycle_check_kernel<<<args.blocks, args.threads>>>(d_checksum, d_workspace, workspace_words);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
    ...
    print_standard_summary("cuckoo_cortex", "split", args.runtime_seconds, args.threads, (unsigned long long)total_launches, (unsigned long long)total_nonces, h_result_count, h_checksum);
