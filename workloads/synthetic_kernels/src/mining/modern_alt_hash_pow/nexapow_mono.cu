...
__global__ void nexapow_mono_kernel(const MiningJob* job, MiningResult* results, unsigned int* result_count, uint32_t* checksum, const uint32_t* dataset, size_t dataset_words) {
...
int main(int argc, char** argv) {
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        ...
        CUDA_CHECK(cudaMemcpy(d_job, &h_job, sizeof(MiningJob), cudaMemcpyHostToDevice));
        nexapow_mono_kernel<<<args.blocks, args.threads>>>(d_job, d_results, d_result_count, d_checksum, d_workspace, workspace_words);
    ...
    print_standard_summary("nexapow", "mono", args.runtime_seconds, args.threads, (unsigned long long)total_launches, (unsigned long long)total_nonces, h_result_count, h_checksum);
