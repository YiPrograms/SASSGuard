...
__global__ void ghostrider_stage_hash_kernel(uint32_t* workspace, size_t words, uint32_t seed) {
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    for (uint64_t i = tid; i < words; i += uint64_t(blockDim.x) * gridDim.x) {
        uint32_t x = avalanche32(uint32_t(i) ^ seed ^ 0x58a05430u);
        workspace[i] = rotl32(x + uint32_t(i * 2654435761ull), int(i & 31));
    }
}

__global__ void ghostrider_stage_scratchpad_kernel(uint32_t* workspace, size_t words, uint32_t seed) {
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    for (uint64_t i = tid; i < words; i += uint64_t(blockDim.x) * gridDim.x) {
        uint32_t x = avalanche32(uint32_t(i) ^ seed ^ 0xcf50f5f2u);
        workspace[i] = rotl32(x + uint32_t(i * 2654435761ull), int(i & 31));
    }
}

__global__ void ghostrider_stage_mix_kernel(uint32_t* workspace, size_t words, uint32_t seed) {
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    for (uint64_t i = tid; i < words; i += uint64_t(blockDim.x) * gridDim.x) {
        uint32_t x = avalanche32(uint32_t(i) ^ seed ^ 0xc79799c4u);
        workspace[i] = rotl32(x + uint32_t(i * 2654435761ull), int(i & 31));
    }
}

__global__ void ghostrider_search_kernel(const MiningJob* job, MiningResult* results, unsigned int* result_count, uint32_t* checksum, const uint32_t* dataset, size_t dataset_words) {
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    uint64_t nonce = job->start_nonce + tid * uint64_t(job->nonce_count);
    uint32_t digest[8];
    blake3_digest(job->header, job->header_len, nonce, digest); groestl_digest(job->header, job->header_len, nonce ^ digest[0], digest); skein_digest(job->header, job->header_len, nonce ^ digest[1], digest); jh_digest(job->header, job->header_len, nonce ^ digest[2], digest); keccak_digest(job->header, job->header_len, nonce ^ digest[3], digest); cubehash_digest(job->header, job->header_len, nonce ^ digest[4], digest); aes_like_digest(job->header, job->header_len, nonce ^ digest[5], digest);
...
__global__ void ghostrider_final_kernel(uint32_t* checksum, const uint32_t* workspace, size_t words) {
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
int main(int argc, char** argv) {
    ...
    ghostrider_stage_hash_kernel<<<args.blocks, args.threads>>>(d_workspace, workspace_words, args.seed + 0x7e2fu);
    CUDA_CHECK(cudaGetLastError());
    ghostrider_stage_scratchpad_kernel<<<args.blocks, args.threads>>>(d_workspace, workspace_words, args.seed + 0x2982u);
    CUDA_CHECK(cudaGetLastError());
    ghostrider_stage_mix_kernel<<<args.blocks, args.threads>>>(d_workspace, workspace_words, args.seed + 0xdb26u);
    CUDA_CHECK(cudaGetLastError());
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        ...
        CUDA_CHECK(cudaMemcpy(d_job, &h_job, sizeof(MiningJob), cudaMemcpyHostToDevice));
        ghostrider_search_kernel<<<args.blocks, args.threads>>>(d_job, d_results, d_result_count, d_checksum, d_workspace, workspace_words);
        CUDA_CHECK(cudaGetLastError());
        ghostrider_final_kernel<<<args.blocks, args.threads>>>(d_checksum, d_workspace, workspace_words);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
    ...
    print_standard_summary("ghostrider", "split", args.runtime_seconds, args.threads, (unsigned long long)total_launches, (unsigned long long)total_nonces, h_result_count, h_checksum);
