...
__global__ void x16rv2_mono_kernel(const MiningJob* job, MiningResult* results, unsigned int* result_count, uint32_t* checksum) {
    uint64_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    uint64_t nonce = job->start_nonce + tid * uint64_t(job->nonce_count);
    uint32_t digest[8];
    seed_digest(job->header, job->header_len, nonce, 0x16u, digest);
    #pragma unroll
    for (int s = 0; s < 16; ++s) {
        uint32_t order = (job->header[s >> 1] >> ((s & 1) * 4)) & 15u;
        switch (order) {
            case 0: blake3_digest(job->header, job->header_len, nonce + s, digest); break;
            case 1: groestl_digest(job->header, job->header_len, nonce + s, digest); break;
            case 2: skein_digest(job->header, job->header_len, nonce + s, digest); break;
            case 3: jh_digest(job->header, job->header_len, nonce + s, digest); break;
            case 4: keccak_digest(job->header, job->header_len, nonce + s, digest); break;
            default: mix_digest_round(digest, order * 0x1020304u + s); break;
        }
    }
...
int main(int argc, char** argv) {
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        ...
        CUDA_CHECK(cudaMemcpy(d_job, &h_job, sizeof(MiningJob), cudaMemcpyHostToDevice));
        x16rv2_mono_kernel<<<args.blocks, args.threads>>>(d_job, d_results, d_result_count, d_checksum);
    ...
    print_standard_summary("x16rv2", "mono", args.runtime_seconds, args.threads, (unsigned long long)total_launches, (unsigned long long)total_nonces, h_result_count, h_checksum);
