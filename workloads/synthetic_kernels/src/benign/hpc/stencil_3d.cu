...
__global__ void stencil_3d_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 49u + 413u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    size_t plane=4096; x=(x+input[(tid+1)%span]+input[(tid+64)%span]+input[(tid+plane)%span]+input[(tid+span-1)%span]+input[(tid+span-64)%span]+input[(tid+span-plane)%span])/7u;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(3576916120u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 3576916120u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        stencil_3d_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("hpc", "stencil_3d", options, total_launches, total_elements, h_checksum);
