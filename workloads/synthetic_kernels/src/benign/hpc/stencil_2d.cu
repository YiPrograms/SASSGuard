...
__global__ void stencil_2d_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 47u + 396u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    size_t w=1024; x=(x*4u + input[(tid+1)%span]+input[(tid+w)%span]+input[(tid+span-1)%span]+input[(tid+span-w)%span])/8u;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(922480359u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 922480359u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        stencil_2d_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("hpc", "stencil_2d", options, total_launches, total_elements, h_checksum);
