...
__global__ void dgemm_naive_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 39u + 328u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    double acc=0.0; for(int k=0;k<24;k++){ acc += double(input[(tid+k)%span]&1023u)*double(input[(tid+k*19)%span]&1023u); } x=uint32_t(acc)+z;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(3189639203u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 3189639203u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        dgemm_naive_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("hpc", "dgemm_naive", options, total_launches, total_elements, h_checksum);
