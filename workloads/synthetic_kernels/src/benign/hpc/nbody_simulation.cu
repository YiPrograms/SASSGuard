...
__global__ void nbody_simulation_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 51u + 430u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    float ax=0, ay=0; for(int j=0;j<32;j++){ float dx=float((input[(tid+j)%span]&255u))-float(x&255u); float inv=rsqrtf(dx*dx+1.0f); ax += dx*inv; ay += inv; } x=uint32_t(ax*1024.0f+ay*17.0f)^z;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(1936384585u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 1936384585u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        nbody_simulation_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("hpc", "nbody_simulation", options, total_launches, total_elements, h_checksum);
