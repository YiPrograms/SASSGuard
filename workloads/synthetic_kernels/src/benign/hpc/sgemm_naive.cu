...
__global__ void sgemm_naive_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 35u + 294u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    float acc=0.0f; for(int k=0;k<32;k++){ acc += float((input[(tid+k)%span]&255u))*float((input[(tid+k*17)%span]&255u)); } x=uint32_t(acc)+z;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(2175734977u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 2175734977u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        sgemm_naive_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("hpc", "sgemm_naive", options, total_launches, total_elements, h_checksum);
