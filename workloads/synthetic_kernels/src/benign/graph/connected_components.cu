...
__global__ void connected_components_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 79u + 668u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    uint32_t label=x; for(int e=0;e<8;e++){ uint32_t nlabel=input[(tid*8u+e)%span]; label = nlabel < label ? nlabel : label; } x=label;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(443779575u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 443779575u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        connected_components_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("graph", "connected_components", options, total_launches, total_elements, h_checksum);
