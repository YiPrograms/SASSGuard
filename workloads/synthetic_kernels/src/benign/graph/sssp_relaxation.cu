...
__global__ void sssp_relaxation_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 77u + 651u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    uint32_t dist=x&4095u; for(int e=0;e<8;e++){ uint32_t cand=(input[(tid*8u+e)%span]&4095u)+uint32_t(e); dist = cand < dist ? cand : dist; } x=dist^y;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(2084311110u));
}
...
int main(int argc, char** argv) {
    ...
    size_t n = (options.nodes + options.edges);
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 2084311110u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        sssp_relaxation_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("graph", "sssp_relaxation", options, total_launches, total_elements, h_checksum);
