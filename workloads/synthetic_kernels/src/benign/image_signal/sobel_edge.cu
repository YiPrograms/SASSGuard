...
__global__ void sobel_edge_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 89u + 753u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    size_t w=1024; int gx=int(input[(tid+1)%span]&255u)-int(input[(tid+span-1)%span]&255u); int gy=int(input[(tid+w)%span]&255u)-int(input[(tid+span-w)%span]&255u); int ax=gx<0?-gx:gx; int ay=gy<0?-gy:gy; x=uint32_t(ax+ay);
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(831056492u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 831056492u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        sobel_edge_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("image_signal", "sobel_edge", options, total_launches, total_elements, h_checksum);
