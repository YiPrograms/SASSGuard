...
__global__ void dct_8x8_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 93u + 787u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    float acc=0; for(int k=0;k<8;k++){ acc += float(input[(tid+k)%span]&255u)*cosf((float(k)+0.5f)*float(tid&7u)*0.39269908f); } x=uint32_t(acc*256.0f)^y;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(1844960718u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 1844960718u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        dct_8x8_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("image_signal", "dct_8x8", options, total_launches, total_elements, h_checksum);
