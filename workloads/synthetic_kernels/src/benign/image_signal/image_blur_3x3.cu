...
__global__ void image_blur_3x3_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 87u + 736u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    size_t w=1024; uint32_t acc=0; for(int dy=-1;dy<=1;dy++) for(int dx=-1;dx<=1;dx++) acc += input[(tid + span + dy*w + dx)%span]&255u; x=acc/9u;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(2471588027u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 2471588027u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        image_blur_3x3_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("image_signal", "image_blur_3x3", options, total_launches, total_elements, h_checksum);
