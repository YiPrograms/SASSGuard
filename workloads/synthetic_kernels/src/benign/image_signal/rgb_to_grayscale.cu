...
__global__ void rgb_to_grayscale_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 85u + 719u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    uint32_t r=x&255u,g=(x>>8)&255u,b=(x>>16)&255u; x=uint32_t(0.299f*r+0.587f*g+0.114f*b);
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(4112119562u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 4112119562u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        rgb_to_grayscale_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("image_signal", "rgb_to_grayscale", options, total_launches, total_elements, h_checksum);
