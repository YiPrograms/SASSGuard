...
__global__ void fft_butterfly_synthetic_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 91u + 770u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    float angle=6.2831853f*float(tid&255u)/256.0f; float c=cosf(angle), s=sinf(angle); float ar=float(x&65535u), ai=float(y&65535u); x=uint32_t(ar*c-ai*s)^uint32_t(ar*s+ai*c);
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(3485492253u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 3485492253u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        fft_butterfly_synthetic_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("image_signal", "fft_butterfly_synthetic", options, total_launches, total_elements, h_checksum);
