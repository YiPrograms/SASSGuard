...
__global__ void fir_filter_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 95u + 804u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    float acc=0; for(int k=0;k<32;k++){ acc += float(input[(tid+k)%span]&255u) * (0.03125f*float((k&7)+1)); } x=uint32_t(acc)^y;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(204429183u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 204429183u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        fir_filter_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("image_signal", "fir_filter", options, total_launches, total_elements, h_checksum);
