...
__global__ void random_index_read_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 33u + 277u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    size_t idx=((uint64_t)x*2654435761ull + y) % span; x=input[idx] ^ input[(idx*7u+3u)%span];
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(3816266512u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 3816266512u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        random_index_read_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("memory", "random_index_read", options, total_launches, total_elements, h_checksum);
