...
__global__ void pointer_chase_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 31u + 260u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    size_t idx=tid%span; for(int i=0;i<8;i++){ idx=(input[idx] ^ z) % span; } x=input[idx]^idx;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(1161830751u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 1161830751u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        pointer_chase_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("memory", "pointer_chase", options, total_launches, total_elements, h_checksum);
