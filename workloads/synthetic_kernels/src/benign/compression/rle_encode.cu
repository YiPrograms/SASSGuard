...
__global__ void rle_encode_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 97u + 821u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    uint32_t run=1; for(int i=1;i<8;i++){ run += ((input[(tid+i)%span]&255u)==(x&255u)); } x=(run<<24)^(x&0xffffffu);
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(2858864944u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 2858864944u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        rle_encode_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("compression", "rle_encode", options, total_launches, total_elements, h_checksum);
