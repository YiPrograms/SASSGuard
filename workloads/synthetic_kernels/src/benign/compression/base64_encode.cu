...
__global__ void base64_encode_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 103u + 872u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    uint32_t a=x&0xffffffu; uint32_t o0=(a>>18)&63u,o1=(a>>12)&63u,o2=(a>>6)&63u,o3=a&63u; x=o0|(o1<<8)|(o2<<16)|(o3<<24);
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(2232237635u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 2232237635u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        base64_encode_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("compression", "base64_encode", options, total_launches, total_elements, h_checksum);
