...
__global__ void xxhash_benchmark_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 107u + 906u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    x += y * 0x9e3779b1u; x = (x << 13) | (x >> 19); x *= 0x85ebca77u; x ^= x >> 16;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(3246141861u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 3246141861u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        xxhash_benchmark_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("compression", "xxhash_benchmark", options, total_launches, total_elements, h_checksum);
