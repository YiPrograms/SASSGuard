...
__global__ void lz_match_find_synthetic_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 99u + 838u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    uint32_t best=0; for(int off=1;off<=16;off++){ uint32_t m=0; for(int j=0;j<4;j++) m += ((input[(tid+j)%span]&255u)==(input[(tid+span-off+j)%span]&255u)); best = m>best?m:best; } x=best^y;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(1218333409u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 1218333409u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        lz_match_find_synthetic_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("compression", "lz_match_find_synthetic", options, total_launches, total_elements, h_checksum);
