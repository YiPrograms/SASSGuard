...
__global__ void crc32_checksum_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 105u + 889u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    uint32_t c=x^0xffffffffu; for(int b=0;b<4;b++){ c ^= (y>>(b*8))&255u; for(int k=0;k<8;k++) c=(c>>1)^(0xedb88320u & (0u-(c&1u))); } x=c^0xffffffffu;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(591706100u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 591706100u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        crc32_checksum_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("compression", "crc32_checksum", options, total_launches, total_elements, h_checksum);
