...
__global__ void chacha20_block_batch_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 119u + 1008u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    uint32_t a=x,b=y,c=z,d=x^0x61707865u; for(int r=0;r<10;r++){ a+=b; d^=a; d=(d<<16)|(d>>16); c+=d; b^=c; b=(b<<12)|(b>>20); } x=a^b^c^d;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(1992887243u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 1992887243u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        chacha20_block_batch_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("crypto_benign", "chacha20_block_batch", options, total_launches, total_elements, h_checksum);
