...
__global__ void keccak_batch_hash_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 113u + 957u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    uint32_t a=x,b=y,c=z,d=x^y,e=y^z; for(int r=0;r<24;r++){ uint32_t t=b ^ ((~c)&d); a^=t^uint32_t(r); b=(b<<1)|(b>>31); c=(c<<7)|(c>>25); d=(d<<12)|(d>>20); e^=a; } x=a^b^c^d^e;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(2619514552u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 2619514552u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        keccak_batch_hash_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("crypto_benign", "keccak_batch_hash", options, total_launches, total_elements, h_checksum);
