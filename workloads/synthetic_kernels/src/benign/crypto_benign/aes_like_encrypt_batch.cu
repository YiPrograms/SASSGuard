...
__global__ void aes_like_encrypt_batch_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 117u + 991u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    for(int r=0;r<10;r++){ x ^= y + uint32_t(r)*0x01010101u; x = ((x & 0x0f0f0f0fu)<<4)|((x & 0xf0f0f0f0u)>>4); x = (x<<8)|(x>>24); y ^= x; }
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(3633418778u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 3633418778u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        aes_like_encrypt_batch_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("crypto_benign", "aes_like_encrypt_batch", options, total_launches, total_elements, h_checksum);
