...
__global__ void random_number_philox_like_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 123u + 1042u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    uint32_t hi=__umulhi(x,0xD256D193u); uint32_t lo=x*0xD256D193u; for(int r=0;r<10;r++){ x=hi^y^uint32_t(r); y=lo; hi=__umulhi(x,0x9E3779B9u); lo=x*0x9E3779B9u; } x^=y;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(3006791469u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 3006791469u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        random_number_philox_like_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("crypto_benign", "random_number_philox_like", options, total_launches, total_elements, h_checksum);
