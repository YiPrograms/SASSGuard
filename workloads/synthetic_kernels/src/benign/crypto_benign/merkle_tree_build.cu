...
__global__ void merkle_tree_build_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 121u + 1025u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    uint32_t left=x,right=y; for(int level=0;level<8;level++){ left ^= right + 0x9e3779b9u + (left<<6) + (left>>2); right ^= left + uint32_t(level); } x=left^right;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(352355708u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 352355708u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        merkle_tree_build_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("crypto_benign", "merkle_tree_build", options, total_launches, total_elements, h_checksum);
