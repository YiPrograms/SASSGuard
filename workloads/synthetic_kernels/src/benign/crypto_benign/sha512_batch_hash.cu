...
__global__ void sha512_batch_hash_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 111u + 940u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    uint64_t a=((uint64_t)x<<32)|y; uint64_t b=((uint64_t)y<<32)|z; for(int r=0;r<24;r++){ a=((a>>28)|(a<<36))+((a&b)^~a)+0x6a09e667f3bcc908ull; b^=a+uint64_t(r); } x=uint32_t(a)^uint32_t(b>>32);
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(4260046087u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 4260046087u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        sha512_batch_hash_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("crypto_benign", "sha512_batch_hash", options, total_launches, total_elements, h_checksum);
