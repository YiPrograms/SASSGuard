...
__global__ void blake2b_batch_hash_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 115u + 974u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    uint64_t a=((uint64_t)x<<32)|y,b=((uint64_t)y<<32)|z; for(int r=0;r<12;r++){ a=a+b+0x6a09e667f3bcc908ull; b=((b^a)>>32)|((b^a)<<32); } x=uint32_t(a)^uint32_t(b>>32);
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(978983017u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 978983017u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        blake2b_batch_hash_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("crypto_benign", "blake2b_batch_hash", options, total_launches, total_elements, h_checksum);
