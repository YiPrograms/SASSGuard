...
__global__ void sha256_batch_hash_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 109u + 923u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    for(int r=0;r<32;r++){ uint32_t ch=(x&y)^(~x&z); uint32_t maj=(x&y)^(x&z)^(y&z); x=((x>>2)|(x<<30))+ch+0x6a09e667u+uint32_t(r); y=((y>>13)|(y<<19))+maj; }
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(1605610326u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 1605610326u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        sha256_batch_hash_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("crypto_benign", "sha256_batch_hash", options, total_launches, total_elements, h_checksum);
