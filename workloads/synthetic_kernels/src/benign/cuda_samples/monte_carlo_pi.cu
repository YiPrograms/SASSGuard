...
__global__ void monte_carlo_pi_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 19u + 158u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    float fx = ((x & 65535u) / 65535.0f) * 2.0f - 1.0f; float fy = ((y & 65535u) / 65535.0f) * 2.0f - 1.0f;
    x = (fx * fx + fy * fy <= 1.0f) ? 1u : 0u;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(2415085369u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 2415085369u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        monte_carlo_pi_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("cuda_samples", "monte_carlo_pi", options, total_launches, total_elements, h_checksum);
