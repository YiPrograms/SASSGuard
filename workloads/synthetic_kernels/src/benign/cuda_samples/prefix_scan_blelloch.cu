...
__global__ void prefix_scan_blelloch_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 13u + 107u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    __shared__ uint32_t s[256]; int lane = threadIdx.x & 255; s[lane] = x & 1023u; __syncthreads();
    for (int off = 1; off < 256; off <<= 1) { uint32_t v = lane >= off ? s[lane - off] : 0; __syncthreads(); s[lane] += v; __syncthreads(); }
    x = s[lane] ^ y;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(3041712678u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 3041712678u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        prefix_scan_blelloch_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("cuda_samples", "prefix_scan_blelloch", options, total_launches, total_elements, h_checksum);
