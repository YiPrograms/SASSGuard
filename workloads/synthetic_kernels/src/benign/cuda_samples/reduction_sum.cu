...
__global__ void reduction_sum_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 11u + 90u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    __shared__ uint32_t s[256]; int lane = threadIdx.x & 255; s[lane] = x + y; __syncthreads();
    for (int off = 128; off > 0; off >>= 1) { if (lane < off) s[lane] += s[lane + off]; __syncthreads(); }
    x = s[0] ^ z;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(387276917u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 387276917u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        reduction_sum_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("cuda_samples", "reduction_sum", options, total_launches, total_elements, h_checksum);
