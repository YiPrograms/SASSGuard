...
__global__ void matrix_transpose_tiled_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 9u + 73u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    __shared__ uint32_t tile[16][17];
    int lane = threadIdx.x & 255; tile[lane >> 4][lane & 15] = x ^ y; __syncthreads();
    x = tile[lane & 15][lane >> 4] + z;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(2027808452u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 2027808452u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        matrix_transpose_tiled_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("cuda_samples", "matrix_transpose_tiled", options, total_launches, total_elements, h_checksum);
