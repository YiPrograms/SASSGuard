...
__global__ void bitonic_sort_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 17u + 141u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    uint32_t a=x,b=y; for(int k=2;k<=32;k<<=1){ bool up=((tid & k)==0); if((a>b)==up){uint32_t t=a;a=b;b=t;} a^=(b<<1)|(b>>31); } x=a^b;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(4055616904u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 4055616904u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        bitonic_sort_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("cuda_samples", "bitonic_sort", options, total_launches, total_elements, h_checksum);
