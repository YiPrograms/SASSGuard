...
__global__ void attention_apply_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 69u + 583u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    float acc=0; for(int k=0;k<64;k++){ float p=expf(float(input[(tid+k)%span]&31u)/32.0f); acc=fmaf(p,float(input[(tid+k*5)%span]&255u),acc); } x=uint32_t(acc)^z;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(56502658u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 56502658u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        attention_apply_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("ai", "attention_apply", options, total_launches, total_elements, h_checksum);
