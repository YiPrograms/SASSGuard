...
__global__ void matmul_fp32_tiled_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 53u + 447u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    __shared__ uint32_t a[256]; int lane=threadIdx.x&255; a[lane]=x; __syncthreads(); float acc=0; for(int k=0;k<32;k++){ acc=fmaf(float(a[(lane+k)&255]&255u),float(y&255u),acc); } x=uint32_t(acc);
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(295853050u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 295853050u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        matmul_fp32_tiled_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("ai", "matmul_fp32_tiled", options, total_launches, total_elements, h_checksum);
