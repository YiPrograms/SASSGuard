...
__global__ void conv2d_direct_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 55u + 464u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    size_t w=1024; uint32_t acc=0; for(int dy=-1;dy<=1;dy++) for(int dx=-1;dx<=1;dx++) acc += input[(tid + span + dy*w + dx)%span]; x=acc/9u;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(2950288811u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 2950288811u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        conv2d_direct_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("ai", "conv2d_direct", options, total_launches, total_elements, h_checksum);
