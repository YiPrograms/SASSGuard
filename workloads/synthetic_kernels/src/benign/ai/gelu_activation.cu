...
__global__ void gelu_activation_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 59u + 498u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    float v=float(int32_t(x & 65535u)-32768)/8192.0f; float g=0.5f*v*(1.0f+tanhf(0.79788456f*(v+0.044715f*v*v*v))); x=uint32_t(g*100000.0f)^y;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(3964193037u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 3964193037u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        gelu_activation_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("ai", "gelu_activation", options, total_launches, total_elements, h_checksum);
