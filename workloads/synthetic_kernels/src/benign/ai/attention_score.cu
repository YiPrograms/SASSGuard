...
__global__ void attention_score_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 67u + 566u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    float acc=0; for(int k=0;k<64;k++){ acc=fmaf(float(input[(tid+k)%span]&255u),float(input[(tid+k*3)%span]&255u),acc); } x=uint32_t(acc*0.125f)^z;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(1697034193u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 1697034193u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        attention_score_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("ai", "attention_score", options, total_launches, total_elements, h_checksum);
