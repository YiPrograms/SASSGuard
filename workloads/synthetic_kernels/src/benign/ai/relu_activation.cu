...
__global__ void relu_activation_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 57u + 481u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    int32_t v=int32_t(x); x = v < 0 ? 0u : uint32_t(v);
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(1309757276u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 1309757276u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        relu_activation_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("ai", "relu_activation", options, total_launches, total_elements, h_checksum);
