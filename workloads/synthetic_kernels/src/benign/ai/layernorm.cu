...
__global__ void layernorm_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 61u + 515u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    __shared__ float s[256]; int lane=threadIdx.x&255; float v=float(x&1023u); s[lane]=v; __syncthreads(); for(int off=128;off>0;off>>=1){ if(lane<off) s[lane]+=s[lane+off]; __syncthreads(); } float mean=s[0]/256.0f; float r=(v-mean)*rsqrtf(fabsf(mean)+1.0f); x=uint32_t(r*4096.0f)^y;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(2323661502u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 2323661502u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        layernorm_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("ai", "layernorm", options, total_launches, total_elements, h_checksum);
