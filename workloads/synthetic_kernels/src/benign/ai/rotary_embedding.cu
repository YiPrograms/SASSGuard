...
__global__ void rotary_embedding_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 71u + 600u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    float angle=float(tid&1023u)*0.001f; float c=cosf(angle), s=sinf(angle); float a=float(x&65535u), b=float(y&65535u); x=uint32_t(a*c-b*s)^uint32_t(a*s+b*c);
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(2710938419u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 2710938419u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        rotary_embedding_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("ai", "rotary_embedding", options, total_launches, total_elements, h_checksum);
