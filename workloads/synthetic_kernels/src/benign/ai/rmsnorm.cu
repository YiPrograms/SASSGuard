...
__global__ void rmsnorm_kernel(const uint32_t* input, uint32_t* output, uint32_t* checksum, size_t n, size_t span) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n) return;
    uint32_t x = input[tid];
    uint32_t y = input[(tid * 63u + 532u) % span];
    uint32_t z = static_cast<uint32_t>(tid) ^ 0x9e3779b9u;
    __shared__ float s[256]; int lane=threadIdx.x&255; float v=float(x&1023u); s[lane]=v*v; __syncthreads(); for(int off=128;off>0;off>>=1){ if(lane<off) s[lane]+=s[lane+off]; __syncthreads(); } x=uint32_t(v*rsqrtf(s[0]/256.0f+1e-5f)*4096.0f)^y;
    output[tid] = x;
    atomicAdd(checksum, x ^ z ^ static_cast<uint32_t>(683129967u));
}
...
int main(int argc, char** argv) {
    ...
    benign_init_u32<<<init_blocks, options.cli.threads>>>(d_input, span, options.cli.seed + 683129967u);
    ...
    while (std::chrono::steady_clock::now() < end_time) {
        rmsnorm_kernel<<<options.cli.blocks, options.cli.threads>>>(d_input, d_output, d_checksum, n, span);
        CUDA_CHECK(cudaGetLastError());
        total_launches++;
        total_elements += n;
    ...
    print_benign_summary("ai", "rmsnorm", options, total_launches, total_elements, h_checksum);
